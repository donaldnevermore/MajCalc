...
import { count_chitoitsu_shanten, count_kokushi_shanten } from "./special-hands";
import { compare_tile } from "./tile";
import { generate_chitoitsu_hora, generate_hora, generate_kokushi_hora } from "./yaku";
...
const dedupe_hora = (hora: Hora[], tableConfig: TableConfig, ruleConfig: RuleConfig): HoraWithPoint[] => {
  ...
  hp.sort((a, b) => {
    ...
    if (a.pointSet.type === "yakuman" && b.pointSet.type === "yakuman") return b.pointSet.point - a.pointSet.point;
    if (a.pointSet.type === "normal" && b.pointSet.type === "normal") return b.pointSet.han - a.pointSet.han;
    return 0;
  });
  ...
  for (const a of hp) {
    ...
    if (compare_tile(a.hora.tile, last.hora.tile) !== 0 || a.hora.type !== last.hora.type) {
      result.push(a);
    }
  }
  return result;
...
export const generate_result = (
  ...
  if ("melds" in hand) {
    const { results, shanten } = deconstructure_and_count_shanten(hand.legal, hand.melds.length);
    if (shanten !== 0) {
      return { tempai: false, shanten };
    }
    const hp = dedupe_hora(
      results.flatMap((pattern) => generate_hora(tableConfig, pattern, hand.melds, handConfig, ruleConfig)),
      tableConfig,
      ruleConfig
    );
    ...
    if (shanten.shanten === 0 || shantenChitoitsu === 0 || shantenKokushi === 0) {
      const hp = dedupe_hora(
        [
          ...(shanten.shanten === 0
            ? shanten.results.flatMap((pattern) => generate_hora(tableConfig, pattern, [], handConfig, ruleConfig))
            : []),
          ...(shantenChitoitsu === 0 ? generate_chitoitsu_hora(tableConfig, hand, handConfig) : []),
          ...(shantenKokushi === 0 ? generate_kokushi_hora(tableConfig, hand, handConfig, ruleConfig) : []),
        ],
        tableConfig,
        ruleConfig
      );
