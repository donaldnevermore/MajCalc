...
export const tile_equals = (left: Tile, right: Tile): boolean => compare_tile(left, right) === 0;
...
export const is_tile = (tile: unknown): tile is Tile => {
  ...
  if (t.type === "character" || t.type === "dots" || t.type === "bamboo") {
    ...
    if (Number.isInteger(t.number) && t.number >= 1 && t.number <= 9) return true;
  }
...
export const is_wind_tile = (tile: Tile): tile is WindTile =>
  tile.type === "east" || tile.type === "south" || tile.type === "west" || tile.type === "north";

export const is_dragon_tile = (tile: Tile): tile is DragonTile =>
  tile.type === "white" || tile.type === "green" || tile.type === "red";

export const is_honor_tile = (tile: Tile): tile is WindTile | DragonTile => is_wind_tile(tile) || is_dragon_tile(tile);

export const is_roto_tile = (tile: Tile): boolean => !is_honor_tile(tile) && (tile.number === 1 || tile.number === 9);

export const is_tanyao_tile = (tile: Tile): boolean => !is_honor_tile(tile) && !is_roto_tile(tile);
...
export const is_honitsu = (tiles: Tile[]): boolean => {
  ...
  return h > 0 && ((c > 0 && d === 0 && b === 0) || (c === 0 && d > 0 && b === 0) || (c === 0 && d === 0 && b > 0));
};
