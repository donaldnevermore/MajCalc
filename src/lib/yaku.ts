...
import { compare_parts, instantiate_part, make_mentsu_from_tatsu } from "./deconstructure";
import type { Kanchan, Kotsu, Mentsu, Pattern, Penchan, Ryammen, Shuntsu, Tatsu, Toitsu } from "./deconstructure";
...
const generate_hora_with_tile = (
  ...
  const shuntsu = [
    ...
    ...melds.filter((m) => m.type === "chow").map((c) => ({ type: "shuntsu", first: (c as Chow).first } as Shuntsu)),
  ]
  ...
  if (
    (["white", "green", "red"] as const).map((type) => tile_to_string({ type })).every((ts) => (counts[ts] || 0) >= 3)
  ) {
    yakuman.push({
      tsumo: true,
      ron: true,
  ...
  if (
    tiles.every(
      (tile) =>
        (tile.type === "character" || tile.type === "dots" || tile.type === "bamboo") &&
  ...
  if (isMenzen) yaku.push({ tsumo: true, ron: false, yaku: { name: "tsumo", point: 1 } });
  if (tiles.every(is_tanyao_tile)) yaku.push({ tsumo: true, ron: true, yaku: { name: "tanyao", point: 1 } });
  if (
    melds.length === 0 &&
    parts.every(
      (p) =>
        ...
        (p.type === "toitsu" &&
          (is_wind_tile(p.tile)
            ? p.tile.type !== tableConfig.round && p.tile.type !== tableConfig.seat
  ...
  if (isMenzen) {
    ...
    for (const s of shuntsu) {
      if (typeof first === "undefined" || compare_parts(first, s) !== 0) first = s;
  ...
  if (
    ...
    (tatsu.type === "kanchan" || tatsu.type === "penchan" || tatsu.type === "ryammen")
  )
  ...
  if (
    (["character", "dots", "bamboo"] as const).some((type) =>
      ([1, 4, 7] as const).every((first) => shuntsu.some((s) => s.first.type === type && s.first.number === first))
    )
  ) {
    yaku.push({
      tsumo: true,
  ...
  if (
    parts.every(
      (p) =>
        (p.type === "shuntsu" && (p.first.number === 1 || p.first.number === 7)) ||
  ...
  if (
    (["white", "green", "red"] as const).map((type) => tile_to_string({ type })).every((ts) => (counts[ts] || 0) >= 2)
  ) {
    yaku.push({
      tsumo: true,
      ron: true,
  ...
  if (
    ...
    (["character", "dots", "bamboo"] as const).some((type) => tiles.every((t) => is_honor_tile(t) || t.type === type))
  ) {
    yaku.push({
      tsumo: true,
      ron: true,
  ...
  if ((["character", "dots", "bamboo"] as const).some((type) => tiles.every((t) => t.type === type))) {
    yaku.push({
      tsumo: true,
      ron: true,
...
export const generate_hora = (
  ...
  if (typeof ryammen !== "undefined") {
    ...
    return waitingTiles.flatMap((t) =>
      generate_hora_with_tile(
        tableConfig,
        pattern.parts.filter((p) => compare_parts(p, ryammenTatsu) !== 0) as (Mentsu | Toitsu)[],
        ryammenTatsu,
        t,
        melds,
        handConfig,
  ...
  if (typeof penchan !== "undefined") {
    ...
    return generate_hora_with_tile(
      tableConfig,
      pattern.parts.filter((p) => compare_parts(p, penchanTatsu) !== 0) as (Mentsu | Toitsu)[],
      penchanTatsu,
      waitingTile,
      melds,
      handConfig,
  ...
  if (typeof kanchan !== "undefined") {
    ...
    return generate_hora_with_tile(
      tableConfig,
      pattern.parts.filter((p) => compare_parts(p, kanchanTatsu) !== 0) as (Mentsu | Toitsu)[],
      kanchanTatsu,
      waitingTile,
      melds,
      handConfig,
  ...
  if (shampon.length === 2) {
    return (shampon as Toitsu[]).flatMap((t) =>
      (counts[tile_to_string(t.tile)] ?? 0) === 4
        ...
        : generate_hora_with_tile(
            tableConfig,
            pattern.parts.filter((p) => compare_parts(p, t) !== 0) as (Mentsu | Toitsu)[],
            t,
            t.tile,
            melds,
            handConfig,
...
export const generate_chitoitsu_hora = (tableConfig: TableConfig, hand: LegalHand, handConfig: HandConfig): Hora[] => {
  const counts = count_tiles(hand.legal);
  const waitingTile = string_to_tile(
    (Object.keys(counts) as (keyof typeof counts)[]).filter((ts) => counts[ts] === 1)[0]
  );
  ...
  if (yakuman.length > 0) {
    if (yakuman.some((y) => y.name === "tenho" || y.name == "chiho")) {
      return [{ form: "chitoitsu", type: "tsumo", tile: waitingTile, yaku: yakuman }];
    } else {
  ...
  if (hand.legal.every(is_tanyao_tile)) yaku.push({ tsumo: true, ron: true, yaku: { name: "tanyao", point: 1 } });
  if (handConfig.last) {
    yaku.push({ tsumo: true, ron: false, yaku: { name: "haitei", point: 1 } });
    yaku.push({ tsumo: false, ron: true, yaku: { name: "hotei", point: 1 } });
  }
  ...
  if (is_honitsu(hand.legal)) yaku.push({ tsumo: true, ron: true, yaku: { name: "honitsu", point: 3 } });
...
export const generate_kokushi_hora = (
  ...
  return product2(["tsumo" as const, "ron" as const], tiles).map(([type, tile]) => ({
    form: "kokushi",
    type,
    tile,
    yaku: [
...
const tenho_or_chiho = (tableConfig: TableConfig): Yaku =>
  tableConfig.seat === "east" ? { name: "tenho", point: 1 } : { name: "chiho", point: 1 };
