...
export const count_kokushi_shanten = (legal: LegalHand["legal"]): number => {
  ...
  const has_toitsu = kokushi_tile_string.some((s) => typeof counts[s] !== "undefined" && (counts[s] as number) >= 2);
...
export const count_chitoitsu_shanten = (legal: LegalHand["legal"]): number => {
  const counts = count_tiles(legal);
  const c = (Object.keys(counts) as (keyof typeof counts)[]).map((k) => counts[k] || 0);

  return 6 - c.filter((n) => n >= 2).length + (7 - Math.min(7, c.filter((n) => n >= 1).length));
};
